...
extern u8 ID_register;    //设置ID寄存器地址 
extern u8 ID1_register;   
extern u8 ID2_register;
extern u8 ID3_register;
...
u16 VCOM_default = 0x6F;

u8 STEP=30;   //采样数组上限
u8 VCOMDH = 0xA6;   //手动调整起始地址
u8 VCOMDC = 0x5f;	  //自动模式起始地址	 

u16 DATA_SUM1[30],DATA_SUMY[30],SURE[30];
//该型号56H寄存器定义VCOM为默认值还是烧录值，故，在烧录前使用默认值初始化，
u8 MTP_OVER=0; 								//在烧录完成重新启动屏幕前使用，烧录值初始化     0--表示未作烧录动作   1--表示烧录动作完成
//======================================END define===================================

u16 count1=0,count2=0;
u16 VCOMDC1=0x0000;
u16 OTP_ERROR_MESAGE=0;

////////////////定义一些常用函数名，便于移植和对接。。
static void DelayMs(u16 time_ssss)
{
	delay_ms(time_ssss);
}

static void BEEP_Dudu(void)
{
	LCD1602_display_Line(Line1602_0,"OTP ERROR       ");
  
}


//static void SSD2828_ENTER_LP_mode(void)
//{
//	SSD2828_ENTER_LP_mode();
//}

static void SSD2828_inition_lcd(void)
{
	MIPI_LCD_INITION();
}

//static void SSD2828_VIDEO_MODE_HS(void)
//{
//	SSD2828_VIDEO_MODE_HS();
//}


static void LCD_RST(bool ddd)
{
	SSD2828_RST(ddd);
}
...
//		ShowTxt("60,20,100,OTP 烧录校验错，可能烧录次数超限");	  DelayMs(100);
//		while(1)
		{BEEP_Dudu();}
        OTP_ERROR_MESAGE++;
}
...
////	Forward_scan();		DelayMs(210);//正掃，該函數可選擇性去除
//	{
//		ShowTxt("60,20,100,OTP 数值超限，请重试或检查光感"); 	 DelayMs(100);
//		while(1)
		{BEEP_Dudu();}
        OTP_ERROR_MESAGE++;
//	}
...
//		ShowTxt("60,20,100,OTP 数值异常,请正确放置光感"); 		DelayMs(100);
//		while(1)
		{BEEP_Dudu();}
//	}
}
...
//	ShowTxt("60,20,700,採樣值有異常");DelayMs(20);
//	while(1)
	{BEEP_Dudu();}
    OTP_ERROR_MESAGE++;
}
...
//	ShowTxt("60,20,100, OTP未烧入！");	  DelayMs(100);
//	while(1)
	{BEEP_Dudu();}
    OTP_ERROR_MESAGE++;
}
...
//	ShowTxt("60,20,100,OTP  OK！");	      DelayMs(100);
    OTP_ERROR_MESAGE=0;
}
...
//	ShowData("32,150,50",senser_val) ;		   DelayMs(150);
//	ShowTxt("32,0,0, VCOM值");	  				   DelayMs(150);
//	ShowData_hex("32,150,0",VCOMDC1) ;		   DelayMs(150);
}
...
//	ShowData_hex("32,180,100",ID1_VALUE) ;		   DelayMs(120);	
//	ShowData_hex("32,280,100",ID2_VALUE) ;		   DelayMs(120);
//	ShowData_hex("32,380,100",ID3_VALUE) ;		   DelayMs(120);
}
//-------------------------------显示区结束---------------------------------------
...
{
	READ_IC_A(0xD9,0x00);
	OTP_VALUE1= MIPI_READ_DATA[1];
	
//	  MIPI_SPI_Write( 4, 0xFF, 0x98, 0x81, 0x04); 
//	  READ_IC(VCOM_Read_reg,0x00); 
//		OTP_VALUE1 = MIPI_READ_DATA[0];    ///VCOM VALUE 
...
////========================================	
//	
////	Allcolor(1,WHITE_bmp);       DelayMs(1900);
//	light_tep = ave_Bright();		
//	gamma_255 = light_tep;
//	
//	Allcolor(1,BLACK_bmp);        DelayMs(190);
//	Allcolor(1,GURY_96_bmp);      DelayMs(1500);       //从此笔数据开始，亮度开始偏大 
//	light_tep = ave_Bright();		
//	gamma_96 = light_tep;
//	
//	Allcolor(1,GURY_128_bmp);     DelayMs(1500);
//	light_tep = ave_Bright();		
//	gamma_128 = light_tep;
//	
//	Allcolor(1,GURY_160_bmp);     DelayMs(1500);
//	light_tep = ave_Bright();		
//	gamma_160 = light_tep;
//	
//	Allcolor(1,GURY_192_bmp);     DelayMs(1500);
//	light_tep = ave_Bright();		
//	gamma_192 = light_tep;
//	
//	Allcolor(1,GURY_224_bmp);     DelayMs(1500);
//	light_tep = ave_Bright();		
//	gamma_224 = light_tep;
//	
...
////	gamma_96 = light_tep;
//	
////	Allcolor(1,GURY_64_bmp);    DelayMs(1500);      //此笔数据未使用
////	light_tep = ave_Bright();		   
////	gamma_64 = light_tep;
//////	
////	Allcolor(1,GURY_32_bmp);     DelayMs(1500);       //此笔数据未使用
////	light_tep = ave_Bright();		
////	gamma_32 = light_tep;
////	
